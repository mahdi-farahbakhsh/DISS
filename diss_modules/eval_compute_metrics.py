...
from diss_modules.face_detection import FaceRecognition
...
@register_eval_fn('fs_adaface')
class FaceSimilarityAdaFace:
    def __init__(self):
        self.reward_network = AdaFaceReward()

    def __call__(self, x: torch.Tensor, gt: torch.Tensor, **kwargs):
        with torch.no_grad():
            gt_embed = self.reward_network._embeddings(gt)
            x_embed = self.reward_network._embeddings(x)
        return torch.norm(gt_embed - x_embed, dim=1).cpu().numpy()

@register_eval_fn('fs_facenet')
class FaceSimilarityFacenet:
    def __init__(self):
        self.reward_network = FaceRecognition(mtcnn_face=True, norm_order=2).cuda()

    def __call__(self, x: torch.Tensor, gt: torch.Tensor, **kwargs):
        diff_norm = self.reward_network.compute_loss(gt, x)
        return diff_norm.cpu().numpy()
